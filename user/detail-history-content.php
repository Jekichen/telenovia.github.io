<div class="container">
    <div class="card shadow p-5 mb-5">
        ...
        <div class="card-body">
            <?php
            // Validasi koneksi dan query pengguna
            if (!isset($conn)) {
                die("Koneksi database tidak tersedia.");
            }

            $userQuery = mysqli_query($conn, "SELECT * FROM tbl_user WHERE id_akun = '$id_akun'");
            if (!$userQuery) {
                die("Error pada query pengguna: " . mysqli_error($conn));
            }
            ...
            <div class="info">
                ...
                <p><strong>Nomor Telepon :</strong> <?= isset($userData['nomor_telepon']) ? htmlspecialchars($userData['nomor_telepon']) : 'Nomor telepon tidak tersedia'; ?></p>
                <p><strong>No. Diagnosa :</strong> <?= htmlspecialchars($no_regdiagnosa); ?></p>
                <p><strong>Tanggal Diagnosa :</strong> <?= date('d-m-Y'); ?></p>
            </div>
        ...
        <div class="text-center mt-4">
            ...
            <a href="cetak.php?no_regdiagnosa=<?= urlencode($no_regdiagnosa); ?>" class="btn btn-primary" target="_blank">Cetak ke PDF</a>
        </div>
    </div>
</div>